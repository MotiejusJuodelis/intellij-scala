...
trait ScTemplateDefinition extends ScNamedElement with PsiClassAdapter with Typeable {
  ...
  def desugaringSource: Option[ScTemplateDefinition]

  def setDesugaringSource(actualElement: ScTypeDefinition): this.type

  protected def desugaredDefinition: Option[ScTemplateDefinition] = None

  def hasDesugared: Boolean = desugaredDefinition.isDefined
