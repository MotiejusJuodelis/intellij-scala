...
abstract class ScTemplateDefinitionImpl[T <: ScTemplateDefinition] private[impl](stub: ScTemplateDefinitionStub[T],
  ...
  override def desugaringSource: Option[ScTemplateDefinition] = Option(getUserData(originalElemKey))

  override def setDesugaringSource(actualElement: ScTypeDefinition): this.type = {
  ...
  def extendsBlock: ScExtendsBlock = {
    desugaredDefinition.map(_.extendsBlock)
      .getOrElse(physicalExtendsBlock)
  }

  override protected def desugaredDefinition: Option[ScTemplateDefinition] = {
    val mayBeDesugared = getContainingFile match {
      case sf: ScalaFile => !sf.isCompiled && sf.isValid && sf.isMetaEnabled
      case _             => false
    }

    if (mayBeDesugared) desugaredInner
    else None
  }
